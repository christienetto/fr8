<script lang="ts">
  ...
  import { open, BaseDirectory } from '@tauri-apps/plugin-fs'; // Importing Tauri's filesystem API
  import "../styles.css";

  let meetingFiles: string[] = [];
  let meetingName: string | undefined;
  let selectedVideo: string | null = null;
  const uploadDir = "/home/chris/atm/randy-bobandy/src-tauri/src/uploads"; // Set your base upload directory here
  ...
  onMount(async () => {
    ...
    try {
      // Invoke Tauri backend function to get files for the meeting
  ...
  // Handle video selection using Tauri's open file system API
  async function selectVideo(file: string) {
    // Use Tauri plugin to open the selected file (you can adjust this depending on your needs)
    const filePath = `${uploadDir}/${meetingName}/${file}`;
    
    try {
      // Try to open the file from the system using Tauri's API
      const fileHandle = await open(filePath, { dir: BaseDirectory.Local });
      selectedVideo = fileHandle.path; // Assign the file path to selectedVideo
    } catch (error) {
      console.error("Error opening video file: ", error);
      alert("Error selecting video file");
    }
  }
    ...
    {#if selectedVideo && selectedVideo.endsWith('.mp4')}
      <section class="video-section">
        ...
        <video width="100%" controls>
          <source src={`file://${selectedVideo}`} type="video/mp4">
